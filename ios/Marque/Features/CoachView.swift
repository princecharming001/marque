...
struct CoachView: View {
    ...
    @ViewBuilder private var teardownSection: some View {
                ...
                ForEach(Array(store.teardowns.enumerated()), id: \.element.id) { i, t in
                    HStack(spacing: 0) {
                        RoundedRectangle(cornerRadius: 2, style: .continuous)
                            .fill(Palette.accent).frame(width: 3).padding(.vertical, Space.md)
    ...
    @ViewBuilder private var statsSection: some View {
        ...
        VStack(alignment: .leading, spacing: Space.md) {
            ...
            HStack(alignment: .lastTextBaseline, spacing: Space.sm) {
                Text(heroValue).font(Typeface.display(36, .semibold)).tracking(Track.hero)
                    .foregroundStyle(Palette.textPrimary)
                Text(heroLabel).font(AppFont.body).foregroundStyle(Palette.textSecondary)
            }
...
private struct FormatBarRow: View {
    let name: String
    let count: Int
    let maxCount: Int
    var body: some View {
        HStack(spacing: Space.md) {
            Text(name).font(AppFont.body).foregroundStyle(Palette.textPrimary).frame(width: 100, alignment: .leading)
            ...
            .frame(height: 8)
            Text("\(count)").font(AppFont.caption).foregroundStyle(Palette.textTertiary).frame(width: 24, alignment: .trailing)
        }
        .padding(.horizontal, Space.lg).padding(.vertical, Space.md)
    }
}
