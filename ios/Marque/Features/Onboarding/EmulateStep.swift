...
// "Who do you want to sound like?" — multi-select emulation targets. Four
// hand-picked presets work instantly (curated style profiles on the backend,
// no scraping needed). Tap-only (owner, 2026-08-16): the typed "link a creator"
// handle field is gone from ONBOARDING — a keyboard mid-quiz breaks tap-through
// momentum, and a misspelled handle + live profile lookup added a failure state
// to a flow that should never block. Custom creators still exist where they
// belong: Profile → "Creators to watch", after the aha.
struct EmulateStep: View {
    ...
    var body: some View {
        VStack(spacing: Space.md) {
            ...
            Text("You can add any creator later from your Profile.")
