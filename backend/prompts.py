...
def edit_plan_prompt(style: str, transcript_words: list[dict], script: dict, brand: dict,
    ...
    _cov = {"minimal": "The creator prefers MINIMAL b-roll — keep the face on screen; use text "
            ...
            "full": "B-ROLL IS REQUIRED: the creator EXPLICITLY chose b-roll cutaways. Emit a "
                    ...
                    "action, place, or process — expect roughly ONE per 5–8 seconds of speech; an "
    ...
    system = (
        ...
        "Keep face-hiding cutaways SHORT (a named thing or number reads in ~1.5–2s; a process/metaphor ~2–3s) "
        "— vary the length by beat, don't hold every cutaway the same. The assembler adds the J-cut lead, "
        "clamps the hold per `need` (entity/data ~2s, evidence ~2.5s, action/concept ~3s full-frame; panel/"
        "card breathe only slightly, ~2.5–3.5s), spacing (≥3s; ≥2s for high-energy or when b-roll is "
        "required), and hook/CTA protection — keep your ranges tight to the phrase.\n"
