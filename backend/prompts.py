...
# ---------------------------------------------------------------------------
# VOICE_DOCTRINE — the global human-voice rule set. Injected verbatim into EVERY
# system prompt that writes user-facing prose (scripts, hooks, captions, replies,
# insights, coaching, ideas). The em/en dash ban is the load-bearing clause: a
# single dash is the loudest "an AI wrote this" tell in short-form copy, so the
# rule is absolute and is ALSO enforced deterministically by scrub_em_dashes().
# ---------------------------------------------------------------------------
VOICE_DOCTRINE = (
    "VOICE (hard rules, apply to every field you write):\n"
    "- Write like a sharp friend texting, not a brand posting. Casual, warm, direct. "
    "Contractions always. Short sentences over stacked clauses.\n"
    "- Plain words over impressive ones. If a 19-year-old wouldn't say it out loud, rewrite it.\n"
    "- No hype vocabulary: never 'unlock', 'level up', 'game changer', 'elevate', 'dive in', "
    "'buckle up', 'transform', 'revolutionize'. No manufactured slang either.\n"
    "- NEVER use an em dash or en dash anywhere, in any field, ever. Not in titles, not in "
    "hooks, not in body copy, not in replies. Use a comma, a period, or two sentences instead. "
    "If a dash feels natural, end the sentence and start a new one.\n"
    "- Titles are lowercase-leaning sentence case, plain and spoken. Never Title Case, never "
    "colon constructions."
)

# TITLE_DOCTRINE — injected wherever a `title` field is generated. Titles are the
# single most Title-Cased, most clickbait-prone field in the product; the exemplars
# below are the whole lever.
TITLE_DOCTRINE = (
    "TITLES: the title is a plain spoken description of what the video actually says, like you "
...
# SHOTPLAN_RULE — the shotPlan is a note to the AI EDITOR, never to the creator.
# Injected anywhere a shotPlan array is produced.
SHOTPLAN_RULE = (
    "SHOTPLAN: shotPlan is a NOTE TO THE AI EDITOR, not instructions to the creator. The "
    "creator films one take of their face; the editor adds everything else. Each entry is one "
    "editor direction: a b-roll cue to source (keep the cue text a literal searchable phrase), "
    "a punch-in with its trigger line, a caption emphasis moment, a keyed screenshot, or where "
...
_LINE_LEAD_DASH_RE = re.compile(r"(?m)^[ \t]*[—–][ \t]*")
_TRAILING_DASH_RE = re.compile(r"[ \t]*[—–][ \t]*$")


def scrub_em_dashes(text: str) -> str:
    """Rewrite every em/en dash out of a string of GENERATED PROSE.

    The model is told never to emit one (VOICE_DOCTRINE); this is the belt to that
    suspenders, applied to model output right before it reaches the client.

    Heuristic, in order:
      - a line-leading dash becomes a plain "- " bullet marker
      - a trailing dash is dropped
      - a spaced dash (" x — y ") becomes ". " if the next visible char is uppercase,
        otherwise ", "
      - a tight dash ("word—word") becomes "word, word"
      - anything left over becomes ", "
      - "  " collapses to " " and " ," to ","

    ⚠️ NEVER run this on transcript-derived data. AssemblyAI emits a TRAILING em dash
    as its aborted-sentence marker and app/edl.py depends on that marker to detect
    false starts; scrubbing transcript words, EDL dicts, or caption-word arrays would
    silently destroy cut quality. Only ever call it on model-authored prose fields
    (see main._VOICE_SCRUB_FIELDS for the whitelist).
    """
    if not isinstance(text, str) or not text:
        return text
    if not any(d in text for d in _DASHES):
        return text

    out = _LINE_LEAD_DASH_RE.sub("- ", text)
    out = _TRAILING_DASH_RE.sub("", out)

    def _repl(m: re.Match) -> str:
        raw = m.group(0)
        spaced = raw[0].isspace() or raw[-1].isspace()
        tail = m.string[m.end():].lstrip()
        if spaced and tail[:1].isupper():
            return ". "
        return ", "

    out = _SPACED_DASH_RE.sub(_repl, out)
    # anything the pass above could not see (e.g. a dash the regex left behind)
    for d in _DASHES:
        out = out.replace(d, ", ")
...
STYLES = {
    ...
    "faceless": {
        # RETIRED: not in ACTIVE_STYLES, never generated (scripts_prompt coerces to
        # talking_head). Kept only so legacy rows/clients that still carry this style
        # key decode and render.
        "retired": True,
    ...
    "green_screen": {
        ...
        "rubric": (
            "The creator films only their face. The AI EDITOR keys a screenshot / post / chart behind them "
            "afterward, so write `body` as a spoken reaction that EXPLICITLY references what the editor will "
            "key in ('this post says X, here's why that's wrong'). Never ask the creator to find, open, or "
        ...
        "exemplar": (
            ...
            '"cta":"Screenshot this for the next time you see that claim.","shotPlan":["key the referenced post '
            'behind them from frame 1","zoom the keyed asset on \'more frequency always wins\'","drop the key and '
            'hold on face for the verdict","caption emphasis on \'twice a week, hard\'"],'
    ...
    "broll_cutaway": {
        ...
        "rubric": (
            ...
            "so make it a literal, specific phrase ('hands gripping a barbell close up', not 'gym'). The editor "
            "sources every cue from stock automatically; the creator films nothing but their face. Aim for 3 to 5 "
        ...
        "exemplar": (
            ...
            'sliding a small plate onto a barbell].","cta":"Follow for the full program.","shotPlan":["hold on '
            'face for the hook, no b-roll","[broll: hands gripping a loaded barbell close up] on \'starting with '
            'your grip\'","[broll: barbell knurling rotating detail] on \'stops the bar rolling\'","[broll: '
            'rounded-back deadlift silhouette] on \'your lower back rounds\'","[broll: hand sliding a small plate '
            'onto a barbell] on \'add five pounds\'","return to face for the CTA"],"targetSeconds":30,'
            '"predictedScore":86}'
        ),
    },
    "duet_split": {
        ...
        "rubric": (
            ...
            "('when he says...', 'notice she skips...'). The creator films only their own face; the editor places "
        ...
        "exemplar": (
            ...
            '"cta":"Follow for training that keeps you out of the physio.","shotPlan":["let the source clip play '
            '2s on its own audio","freeze the top panel on \'half right\'","release the source clip '
            'under the rebuttal","punch in on \'same growth, half the wreckage\'","full frame on face for the '
            'CTA"],"targetSeconds":29,"predictedScore":85}'
        ),
    },
}
...
def tweak_prompt(edl: dict, transcript_words: list[dict], instruction: str,
    ...
    system = (
        ...
        "- set_music {enabled, url?} — background music. Only a direct track url plays; there is NO "
        "server-side music search (a query-only op is rejected). When the creator wants music but "
        "gave no track, emit NO set_music op — point them to the editor's music picker in your "
        "reply instead. enabled:false removes the current track.\n"
...
SCRIPT_SCHEMA = (
    ...
    '"title": str (≤8 words, plain spoken sentence case; see TITLES rule), "summary": str (one line), "hook": str, '
...
def broll_query_rewrite_system() -> str:
    ...
    return (
        ...
        "- For stock needs (entity/data/evidence/action/concept): output a SHORT LITERAL query — "
        "2–4 words, concrete subject + action, optionally ONE setting/detail word ('person "
        "working' → 'founder typing laptop'; 'lifting form' → 'barbell deadlift closeup'). NEVER a "
        "cinematic scene description — long stylized phrases ('… in a dark neon-lit office') match "
        "stock libraries WORSE than short literal ones. Avoid stock clichés (handshake, faceless "
        "suit, lightbulb-idea).\n"
...
def edit_plan_prompt(style: str, transcript_words: list[dict], script: dict, brand: dict,
    ...
    _cov = {"minimal": "The creator prefers MINIMAL b-roll — keep the face on screen; use text "
            ...
            "full": "B-ROLL IS REQUIRED: the creator EXPLICITLY chose b-roll cutaways. Emit a "
                    ...
                    "NEVER suppress a plausible cue to protect density — relevance and density never "
                    "trade off. Relevance still governs each cue: prefer the creator's own footage; "
                    "where there's none, give a SHORT LITERAL stock `query` (2–4 words, concrete "
                    "subject + action) for the thing named; for entity/data/evidence with no real "
                    "asset set `text` for the card fallback.",
    ...
    system = (
        ...
        "anything to be removed. ONE deliberate pause is a TOOL: after the single biggest claim/payoff line, "
        "leave a ~0.3–0.5s beat for it to land and protect that beat with a `keeps` range — scrubbing every "
        "pause reads as robotic, and fast sections only feel fast against a quiet one.\n"
        ...
        "near-identical sentences in a row. DEFINITION CHECK: a span is only a retake/false start when a "
        "LATER kept span re-delivers essentially the same words — the take's CLOSING lines can never be a "
        "false start (there is nothing after them to re-deliver), and the assembler rejects end-swallowing "
        "cuts that aren't a listed sign-off.\n"
        ...
        "word, never a wind-down. But a REAL CTA (rate/comment/follow/share/link-in-bio) is the PAYOFF, not a "
        "sign-off — KEEP it, tight (final ~2–3s): cut any CTA preamble/wind-up, never the ask itself, and "
        "never the final content sentence before it.\n"
        ...
        "note it in pacing_intent as a separate-video candidate. The viral sweet spot for a single-idea "
        "talking-head clip is ~21–38s; a clean cut landing far above that is usually the second-idea signal, "
        "not a reason to gut the payoff. Do NOT cut real substance, examples, or the "
        ...
        "was buried later in the take (the assembler pulls it forward; it also cold-opens mechanically — the "
        "first spoken word lands within ~0.4s of frame 0 — so pick open_on for CONTENT strength, never to "
        "trim lead-in silence). Disqualify greetings, self-intros, and "
        ...
        "The rule that governs everything: a RELEVANT, concrete visual that matches what's being said "
        "AIDS memory (two retrieval routes); a DECORATIVE or tangential visual HARMS it (it competes with "
        "the point — the 'seductive details' effect). So b-roll quantity was never the lever — relevance is. "
        "Fire a visual ONLY when a relevant concrete asset exists for that beat; if a beat wants a visual "
        "change but has no relevant asset, prefer a PUNCH-IN (a cheaper, face-keeping change) or nothing — "
        "never a decorative clip. Cadence (~1 visual change every 4–5s) is a loose PRIOR, not a target: don't "
        "add b-roll to hit it; a punch-in or caption pop satisfies a cadence slot more cheaply than b-roll.\n"
        "FORMAT-CONDITIONED (read the video_type/brief): EDUCATIONAL / EXPLAINER / how-to → protect "
        "comprehension: HIGH relevance bar, SPARSE b-roll, only literal illustrations of the exact thing said, "
        "no mood/decorative clips. ENTERTAINMENT / STORY / hot-take → attention-first: the relevance bar "
        "relaxes, evocative/mood b-roll is allowed where it fits the beat.\n"
        "B-roll SUPPORTS the face, it never replaces it. Classify each line before covering it:\n"
        "• ILLUSTRABLE → b-roll candidate: the line names a concrete object/place/person, a NUMBER/stat, "
        "a PROCESS/how-to step, or shows EVIDENCE/proof. These land better with a picture.\n"
        "• FACE-PROTECTED → NEVER full-frame b-roll: the hook/opening line, any PUNCHLINE or REVEAL, the "
        "PAYOFF (the moment your `understanding.payoff` names — it exists to be delivered on camera), the "
        ...
        + ("MEMES (`need`:meme) — a shared cultural moment (movie/TV/viral clip or reaction GIF) punctuating "
           "a beat. Place it ON the punchline, hot take, absurd stat, or emotional pivot ('I was shocked…') — "
           "any reaction beat where a knowing wink lands harder than footage. Entertainment takes "
           "(rant/story/hot-take/listicle) may use up to 5 per video; educational/explainer takes up to 2 and "
           "ONLY on genuine punchline/reaction beats, NEVER mid-instruction (a meme on an info beat distracts "
           "from the point). `query` = a canonical reaction the audience knows ('side eye', 'mind blown', "
           "'confused math lady', 'this is fine'). Memes are POP-INS: flash-length, ≤1.5s — the assembler "
           "clamps the hold, renders it as a panel (your face stays for the joke) and pulls the clip from a "
           "licensed library; if none fits, it stays on your face.\n"
        ...
        "`text` to the fallback card copy (the entity name, the number rendered big, or the quote) → the "
        "assembler shows a clean TEXT CARD. A text card always beats a wrong clip; NO relevant asset → stay "
        "on the face, never decorative filler. action/concept MAY use stock — give a SHORT LITERAL `query` "
        "(2–4 words, concrete subject + action, e.g. 'barbell deadlift closeup') — never a cinematic scene "
        "description.\n"
        "TIMING: the b-roll illustrates ONE phrase — mark the START of that phrase precisely; if the phrase "
        "runs long the assembler cuts back to the face EARLY rather than letting the insert outlast its words. "
        "Keep face-hiding cutaways SHORT (a named thing or number reads in ~1.2–1.8s; a process/metaphor "
        "~1.5–2.5s; memes FLASH at 0.5–1.5s) — vary the length by beat, don't hold every cutaway the same "
        "(uniform holds are an AI tell; the assembler also jitters spacing for the same reason). The "
        "assembler adds the J-cut lead, clamps the hold per `need` (entity/data ~1.2–1.8s, evidence ~2.5s, "
        "action/concept ~1.2–2.5s full-frame; panel/card breathe only slightly, ceiling ~3s), spacing "
        "(≥3s; ≥1.5s for high-energy/entertainment), and hook/CTA protection — keep your ranges tight to "
        "the phrase.\n"
        "BUDGET: full-frame b-roll HIDES the face; keep total face-hiding coverage well under ~40% of the "
        "video and total inserts under ~50% (the assembler drops overflow). Prefer well-placed cutaways over "
        "blanket coverage.\n"
        "For each insert pick `mode` (how it's composited):\n"
        "  • full — covers the whole frame (face hidden, ~2–3s). Use when the visual needs detail inspection "
        "(screen recordings, charts, action footage) or the line is pure narration.\n"
        "  • panel — a rounded panel up top; the face stays visible below (~2.5–3.5s). Use when the "
        "speaker's delivery matters at the same time (jokes, opinions, emphasis) — panel/card don't count "
        "against the FACE-HIDING budget, so prefer them when the face should stay on screen.\n"
        ...
        "- hook_text: the on-screen TITLE, shown ~2–5s (until the first sentence ends). 5–8 words, ONE line, "
        "sentence case, phrased as a question or bold claim, NO terminal period, no emoji. REWORD the spoken "
        "hook — compress the promise into a sharper line (verbatim restatement is the documented "
...
def brand_block(brand: dict, posts: list[dict] | None = None) -> str:
    ...
    # Onboarding now collects MULTI-select topics/audiences. The singular niche/audience
    # lines above stay byte-identical (match_niche and the cold-start priors parse them);
    # the extra picks are rendered as their own lines so nothing upstream shifts.
    topics = [str(t).strip() for t in (brand.get('topics') or []) if str(t).strip()]
    if len(topics) > 1:
        lines.append(f"- also covers: {', '.join(topics[:8])}")
    audiences = [str(a).strip() for a in (brand.get('audiences') or []) if str(a).strip()]
    if len(audiences) > 1:
        lines.append(f"- also talking to: {', '.join(audiences[:8])}")
...
def steer_prompt(brand: dict, script: dict, instruction: str,
    ...
    system = (
        ...
        "ADD a new personal event, result, number, client story, or credential (see GROUNDING).\n\n"
        f"{VOICE_DOCTRINE}\n\n"
        f"{TITLE_DOCTRINE}\n\n"
        f"{TALKING_HEAD_MANDATE}\n\n"
        f"{SHOTPLAN_RULE}\n\n"
...
SOCIAL_CAPTION_SYSTEM = (
    ...
    "misspelled tag (#fitnescoach, #deficitstalse) is a dead tag nobody follows, it reaches "
    "zero people. Build tags only from words that appear in the niche/audience/script above, "
    "or from obvious well-known tags in that space. If unsure of a spelling, use a simpler tag.\n\n"
...
VOICE_AGENT_SYSTEM = (
    f"{VOICE_DOCTRINE}\n\n"
    "You are Marque's brand interviewer. In a warm, brief spoken conversation (5 to 7 short turns), help a "
    ...
    "to be known for, their natural tone (are they more the teacher or more one of the crew), 2 to 3 concrete "
...
def mock_next_idea(niche: str, insight: dict | None,
    ...
    if insight:
        ...
        return {
            "title": f"run it back with another {val} take",
            "hook": hook,
            "beats": [
...
VIRALITY_BLOCK = (
    ...
    "- Cadence compounds: 3 to 7 posts/week beats bursts. Consistency + iteration on what the data says beats "
...
CRAFT_RULES_BLOCK = (
    ...
    "em dashes or en dashes ANYWHERE; punctuation is commas, periods, and line breaks."
)
...
BODY_FORMAT_RULE = (
    ...
    "BODY FORMATTING (required): write `body` as 2 to 4 SHORT paragraphs separated by a blank line, "
...
def converse_system(mode: str = "chat", persona: str = "closer", response_length: str = "medium") -> str:
    ...
    voice_style = (
        ...
        "Reply in 2 to 4 short conversational sentences. NO markdown, NO lists, NO emoji, no stage directions. "
    ...
    return (
        ...
        "self-contained sentence. Do NOT store small talk, questions, or anything transient. 0 to 3 updates per turn "
        ...
        "\"style\": one of [" + ", ".join(ACTIVE_STYLES) + "] or \"\", \"count\": 1-3}. "
