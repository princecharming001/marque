...
## Phase 1 — memory + ledger + overlay  (flag: MEMORY_V2)
- [ ] `app/memory_v2.py` — extractor (facts only, insights banned), embed, mem0 reconcile ADD/UPDATE/DELETE
- [ ] memory retrieve: cue-gate + `match_memories` + weighted rank (0.55 sim + 0.25 conf + 0.20 recency) + scope filter
- [ ] `app/recall_ledger.py` — per-turn extraction of assistant proposals; `<prior_recommendations>` block
- [ ] wire fire-and-forget post-turn hooks into `/v1/converse` + script gen (5-block injection)
- [ ] tests: reconcile decisions golden, ledger dedup, keyless mock, call-budget ≤2
